...
public interface ReadingSubscriptionRenewalService {
    ...
    List<Long> listProviderPendingCycleIds(Date now, int limit);
    ...
    ReadingSubscriptionProviderQuery claimProviderQuery(long cycleId, Date now, Date nextQueryAt);
    ...
    ReadingSubscriptionRenewalResult recordProviderPending(long cycleId, int attemptNo,
                                                            String responseCode, Date now);
    ReadingSubscriptionRenewalResult settleProviderQuery(long cycleId, int attemptNo,
                                                          String providerTransactionId, Date now);
    ReadingSubscriptionRenewalResult recordProviderQueryFailure(long cycleId, int attemptNo,
                                                                 String responseCode, Date now);
